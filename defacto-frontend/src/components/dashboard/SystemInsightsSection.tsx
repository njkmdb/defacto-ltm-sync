...
import { getSystemInsights, getSystemConfig } from '@/lib/api/pipeline';
...
export default function SystemInsightsSection() {
  ...
  if (useBigQuery) {
    ragData.push({ name: 'Tier 3 (DWH)', value: insights.rag_stat.dwh });
  }
    ...
    <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6 flex flex-col h-[280px]">
      <div className="flex items-center justify-between mb-4 border-b border-gray-100 pb-3">
        <h2 className="text-lg font-bold text-gray-800 flex items-center gap-2">
          <Activity className="w-5 h-5 text-indigo-600" /> {t('system_insights')}
        </h2>
        <div className="flex gap-1 bg-gray-100 p-1 rounded-lg border border-gray-200">
          <button onClick={() => setActiveTab('COST')} className={`p-1.5 rounded-md transition-colors ${activeTab === 'COST' ? 'bg-white shadow-sm text-indigo-600' : 'text-gray-400 hover:text-gray-600'}`} title={t('insights_cost')}><DollarSign className="w-4 h-4" /></button>
          <button onClick={() => setActiveTab('KEYWORDS')} className={`p-1.5 rounded-md transition-colors ${activeTab === 'KEYWORDS' ? 'bg-white shadow-sm text-indigo-600' : 'text-gray-400 hover:text-gray-600'}`} title={t('insights_keyword')}><Hash className="w-4 h-4" /></button>
          <button onClick={() => setActiveTab('RAG')} className={`p-1.5 rounded-md transition-colors ${activeTab === 'RAG' ? 'bg-white shadow-sm text-indigo-600' : 'text-gray-400 hover:text-gray-600'}`} title={t('insights_rag')}><Zap className="w-4 h-4" /></button>
          <button onClick={() => setActiveTab('RISKS')} className={`p-1.5 rounded-md transition-colors ${activeTab === 'RISKS' ? 'bg-white shadow-sm text-red-600' : 'text-gray-400 hover:text-red-600'}`} title={t('insights_risk')}><AlertTriangle className="w-4 h-4" /></button>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto overflow-x-hidden relative">
        {activeTab === 'COST' && (
          <div className="flex flex-col h-full justify-center px-4 animate-in fade-in zoom-in duration-300">
            <div className="flex items-center justify-between mb-2">
              <span className="text-sm font-bold text-gray-500">{t('insights_token_usage')}</span>
            ...
            <div className="flex items-center justify-between bg-indigo-50 p-4 rounded-xl border border-indigo-100">
              <span className="text-sm font-extrabold text-indigo-800">{t('insights_est_cost')}</span>
        ...
        {activeTab === 'KEYWORDS' && (
          <div className="flex flex-wrap gap-2 animate-in fade-in zoom-in duration-300 content-start">
            {insights.hot_keywords.length === 0 && <p className="text-sm text-gray-400 font-bold m-auto mt-10">{t('insights_no_keyword')}</p>}
        ...
        {activeTab === 'RISKS' && (
          <div className="flex flex-col gap-3 animate-in fade-in zoom-in duration-300">
            {insights.risk_alerts.length === 0 && <p className="text-sm text-gray-400 font-bold m-auto mt-10">{t('insights_no_risk')}</p>}
