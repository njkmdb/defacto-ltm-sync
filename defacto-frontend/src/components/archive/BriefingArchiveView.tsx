...
export default function BriefingArchiveView() {
  ...
  const updateMut = useMutation({
    ...
    onError: (err: any) => alert(err.response?.data?.detail || t('alert_update_fail'))
  });
  ...
  const bulkDeleteMut = useMutation({
    ...
    onError: (err: any) => alert(err.response?.data?.detail || t('alert_bulk_del_fail'))
  });
  ...
  const handleEditSubmit = () => {
    if (!editFormData.executive_summary.trim() || !editFormData.query_text.trim()) return alert(t('alert_req_query_summary'));
    updateMut.mutate({
  ...
  if (isLoading) {
    return <div className="py-20 text-center text-gray-400 font-bold flex flex-col items-center"><RefreshCw className="w-8 h-8 animate-spin mb-3 text-indigo-300" /> {t('state_loading')}</div>;
  }
    ...
    <div className="animate-in fade-in zoom-in duration-300">
          ...
          <div className="flex items-center gap-3 mt-7">
            ...
            {selectedIds.length > 0 && (
              <>
                ...
                <button onClick={handleGoToStudio} className="flex items-center gap-1.5 bg-purple-600 hover:bg-purple-700 text-white px-4 py-1.5 rounded-lg text-sm font-bold transition-colors shadow-md h-10"><Sparkles className="w-4 h-4" /> {t('btn_studio', { count: selectedIds.length })}</button>
                <button onClick={() => { if(confirm(t('confirm_delete_multi_briefing'))) bulkDeleteMut.mutate(selectedIds); }} disabled={bulkDeleteMut.isPending} className="flex items-center gap-1 text-sm bg-red-50 hover:bg-red-100 transition-colors text-red-600 border border-red-200 px-3 py-1.5 h-10 rounded-lg font-bold disabled:opacity-50"><Trash2 className="w-4 h-4" /> {t('btn_delete', { count: selectedIds.length })}</button>
              </>
            )}
          </div>
        </div>
      ...
      {briefingsData?.data?.length === 0 ? (
        <div className="py-20 text-center bg-white rounded-2xl border border-dashed border-gray-300">
          ...
          <p className="text-gray-500 font-bold">{t('state_empty_briefings')}</p>
        </div>
      ) : viewMode === 'CARD' ? (
              ...
              <div key={briefing.briefing_id} onClick={() => toggleSelect(briefing.briefing_id)} onDoubleClick={(e) => { e.stopPropagation(); openEditModal(briefing); }} className={`bg-white rounded-2xl shadow-sm border p-6 flex flex-col gap-4 transition-all cursor-pointer ${isSelected ? 'border-indigo-500 ring-1 ring-indigo-500 bg-indigo-50/20' : 'border-gray-200 hover:shadow-md'}`}>
                <div className="flex items-center justify-between border-b border-gray-100 pb-4">
                  <div className="flex items-center gap-4">
                    ...
                    <span className="text-sm font-medium text-gray-400">Date: {formatDateOnly(briefing.ne_ts)}</span>
                  </div>
                </div>
                <div>
                  <h4 className="text-xs font-bold text-gray-400 mb-2 uppercase flex items-center gap-1"><Search className="w-3.5 h-3.5" /> {t('col_query')}</h4>
                  <p className="text-sm text-gray-800 font-semibold bg-gray-50 p-3 rounded-lg border border-gray-100">{briefing.query_text}</p>
                </div>
                <div>
                  <h4 className="text-xs font-bold text-gray-400 mb-2 uppercase">{t('col_exec_summary')}</h4>
          ...
          <table className="w-full text-left border-collapse whitespace-nowrap">
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                ...
                <th className="p-3 w-24 text-xs font-extrabold text-gray-500 uppercase tracking-wider">{t('col_briefing_id')}</th>
                <th className="p-3 w-24 text-xs font-extrabold text-gray-500 uppercase tracking-wider">{t('col_entity_id')}</th>
                <th className="p-3 w-32 text-xs font-extrabold text-gray-500 uppercase tracking-wider">{t('col_date')}</th>
                <th className="p-3 w-64 text-xs font-extrabold text-gray-500 uppercase tracking-wider">{t('col_query')}</th>
                <th className="p-3 text-xs font-extrabold text-gray-500 uppercase tracking-wider min-w-[300px]">{t('col_exec_summary')}</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-100">
              {briefingsData?.data?.map((briefing: any) => {
                const isSelected = selectedIds.includes(briefing.briefing_id);
                return (
                  <tr key={briefing.briefing_id} onClick={() => toggleSelect(briefing.briefing_id)} onDoubleClick={(e) => { e.stopPropagation(); openEditModal(briefing); }} className={`transition-colors cursor-pointer ${isSelected ? 'bg-indigo-50/50' : 'hover:bg-gray-50'}`} title={t('tooltip_view')}>
                    ...
                    <td className="p-3"><span className="text-[10px] font-bold bg-white border border-gray-200 text-gray-600 px-2 py-1 rounded">{formatDateOnly(briefing.ne_ts)}</span></td>
            ...
            {activeModalTab === 'EDIT' ? (
              <>
                <div className="flex-1 overflow-y-auto p-8 bg-white space-y-6">
                  <div>
                    <label className="text-sm font-extrabold text-gray-800 mb-2 flex items-center gap-1.5"><Search className="w-4 h-4 text-blue-500"/> {t('briefing_query')}</label>
                  ...
                  <div>
                    <label className="text-sm font-extrabold text-gray-800 mb-2 flex items-center gap-1.5"><FileText className="w-4 h-4 text-indigo-500"/> {t('briefing_exec_summary')}</label>
                  ...
                  <div>
                    <label className="text-sm font-extrabold text-gray-800 mb-2 flex items-center gap-1.5"><CheckSquare className="w-4 h-4 text-emerald-500"/> {t('briefing_findings')}</label>
                  ...
                  <div>
                    <label className="text-sm font-extrabold text-red-800 mb-2 flex items-center gap-1.5"><AlertTriangle className="w-4 h-4"/> {t('briefing_risks')}</label>
                  ...
                  <div>
                    <label className="text-sm font-extrabold text-gray-800 mb-2 flex items-center gap-1.5"><Lightbulb className="w-4 h-4 text-amber-500"/> {t('briefing_actions')}</label>
                ...
                <div className="p-5 border-t border-gray-200 bg-gray-50 flex justify-end gap-3 shrink-0">
                  <button onClick={() => setIsBriefingViewerOpen(false)} className="px-6 py-2.5 bg-gray-200 text-gray-700 rounded-lg font-bold hover:bg-gray-300 transition-colors">{t('briefing_close')}</button>
                  <button onClick={handleEditSubmit} disabled={updateMut.isPending} className="flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white px-8 py-2.5 rounded-lg font-bold shadow-md transition-colors disabled:opacity-50">
                    {updateMut.isPending ? <RefreshCw className="w-4 h-4 animate-spin"/> : <Save className="w-4 h-4"/>} {t('briefing_save')}
                  </button>
                </div>
              </>
            ) : (
              <div className="flex-1 overflow-y-auto p-6 bg-gray-100 space-y-4">
                {isAuditLoading ? (
                  <div className="py-20 text-center text-gray-500 font-bold flex flex-col items-center">
                    <RefreshCw className="w-8 h-8 animate-spin mb-3 text-emerald-400" /> {t('briefing_audit_loading')}
                  </div>
                ) : (
                  <>
                    <div className="bg-white p-4 rounded-xl border border-gray-200 shadow-sm flex items-center gap-3 mb-2">
                      <Database className="w-6 h-6 text-gray-400" />
                      <p className="text-sm text-gray-600 font-bold">{t('briefing_audit_desc', { count: auditData?.data?.length || 0 })}</p>
                    </div>
