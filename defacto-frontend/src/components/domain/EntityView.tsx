  ...
  return (
    <>
      ...
      <div className="w-full overflow-x-auto bg-white rounded-2xl shadow-sm border border-gray-200">
        <DomainDataTable 
           ...
           openEditModal={(row: any) => {
              ...
              setEditModalData({ id: row.entity_id, name: row.entity_name, type: row.entity_type, parent_id: row.parent_entity_id || '', status_id: row.entity_status_id || 1, aliases, attributes: attrs, ne_ts: row.ne_ts, up_ts: row.up_ts });
              setIsModalOpen(true);
           }}
        />
      </div>
      ...
      <BulkPreviewModal 
        ...
        onConfirm={() => { if(previewErrors === 0) bulkUpsertMut.mutate(previewData.map(item => ({ entity_id: item.id, entity_type: item.type.toUpperCase(), entity_name: item.name, parent_entity_id: item.parent_id, attributes: item.attributes }))); }}
      ...
      <DomainEditModal
        ...
        onSave={(data) => {
            const payload = { entity_id: data.id ? Number(data.id) : null, entity_name: data.name, entity_type: data.type.toUpperCase(), parent_entity_id: data.parent_id ? Number(data.parent_id) : null, entity_status_id: data.status_id, attributes: data.finalAttributes };
