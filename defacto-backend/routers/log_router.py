...
def delete_event_log(log_id: int, db: Session = Depends(get_db)):
    try: return log_service.delete_event_log(log_id, db)
...
def delete_bulk_event_logs(request: BulkDeleteLogRequest, db: Session = Depends(get_db)):
    try: return log_service.delete_bulk_event_logs(request.log_ids, db)
    except Exception as e: raise HTTPException(status_code=500, detail=str(e))
...
def get_event_log_by_id(log_id: int, db: Session = Depends(get_db)):
    try:
        return log_service.get_event_log(log_id, db)
