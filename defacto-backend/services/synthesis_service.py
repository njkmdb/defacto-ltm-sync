...
def execute_fetch_detailed_facts(db: Session, event_ids: List[int], base_entity_id: int, override_limit: bool = False) -> str:
    ...
    facts = query.limit(MAX_FETCH_LIMIT).all() if not override_limit else query.all()
    
    if not facts:
        return "[시스템 알림: 요청한 ID에 해당하는 팩트 원문을 찾을 수 없습니다]"
...
def execute_fetch_entity_master(db: Session, entity_ids: List[int]) -> str:
    ...
    for e in entities:
        details.append(f"► [주체 ID: {e.entity_id} | 타입: {e.entity_type} | 이름: {e.entity_name}]\n- 속성(Attributes): {json.dumps(e.attributes, ensure_ascii=False)}")
    return "\n\n".join(details)
...
def execute_fetch_object_master(db: Session, object_ids: List[int]) -> str:
    ...
    for o in objects:
        details.append(f"► [객체 ID: {o.object_id} | 타입: {o.object_type} | 이름: {o.object_name}]\n- 속성(Attributes): {json.dumps(o.attributes, ensure_ascii=False)}")
    return "\n\n".join(details)
...
async def process_synthesize_context(request: SynthesizeContextRequest, db: Session) -> SynthesizeContextResponse:
    ...
    today_memories = db.query(EventMemory).filter(
        ...
        EventMemory.event_date == request.reference_date,
    ...
    if target_entity_names:
        search_query_text += f" [핵심 타겟: {', '.join(target_entity_names)}] 연관 과거 이력 검색"
    
    if not search_query_text.strip():
        search_query_text = "최근 발생한 중요 이벤트 및 비즈니스 활동 이력 검색"

    query_vector = await embedding_service.get_embedding(search_query_text)
    
    rag_results = rag_service.get_optimal_context(
    ...
    if getattr(request, 'use_deep_search', False):
        logger.info("🔍 [Deep Search Mode] 에이전트 스캔 우회, 연관된 모든 팩트 강제 인출")
        all_ids = set()
        ...
        final_ids_to_fetch = list(all_ids)
        fetched_details = execute_fetch_detailed_facts(db, final_ids_to_fetch, request.base_entity_id, override_limit=True)
        rag_metrics_type = "Deep Search (All Facts Extracted)"
    else:
        logger.info("🤖 [Agent] 1차 Index 스캔 및 마스터/팩트 원문 필요성 판별 중 (Planning)...")
        # 💡 [핵심 교정] 온도/길이 언패킹 적용
        ...
        if required_fact_ids:
            facts_text = execute_fetch_detailed_facts(db, list(required_fact_ids), request.base_entity_id)
            additional_context_payload += f"\n\n{facts_text}"
            
        fetched_details = additional_context_payload
        if not metrics_tools_used:
            rag_metrics_type = "Agentic Dual-Track RAG (SUFFICIENT_INFO)"
        else:
            rag_metrics_type = f"Agentic Tool Calls: {', '.join(metrics_tools_used)}"

    logger.info("📝 [Agent] 2차 인출된 팩트 원문 및 마스터를 바탕으로 최종 일지 합성 중 (Synthesis)...")
    final_payload = f"{index_payload}\n\n{fetched_details}"
    ...
    return SynthesizeContextResponse(
        ...
        data=SynthesizeContextData(
            log_id=999, 
            synthesized_data=SynthesizedData(llm_summary=synthesized_result.llm_summary, action_items=synthesized_result.action_items),
            rag_metrics=RagMetrics(cache_hit=False, memory_type_used=rag_metrics_type)
        )
    )