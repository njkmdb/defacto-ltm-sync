...
async def process_structure_events(request: StructureEventsRequest, db: Session) -> StructureEventsResponse:
    ...
    for raw_id in request.target_raw_ids:
        ...
        if not raw_record:
            results.append(StructureEventResult(raw_id=raw_id, sync_status_id=2, error_reason="원본 데이터 없음"))
            fail_count += 1
            continue
        ...
        try:
            ...
            # 💡 [핵심 교정] 4개의 반환값을 온전히 언패킹하고 물리적 토큰/온도 제어 적용
            ...
            structured_data = await lrse_client.extract_fact(
                ...
                target_schema_cls=target_schema_cls,
            ...
            new_fact = EventFact(
                ...
                target_entity_id=structured_data.ref_entity_id_1 or 0,
                target_object_id=0,
                event_date=raw_record.event_date, 
                schema_name=target_schema_cls.__name__,
                fact_content=structured_data.fact_content,
            ...
            new_memory = EventMemory(
                ...
                event_date=raw_record.event_date,
                memory_type='LTM', 
                content_text=structured_data.content_text, 
                core_keywords=structured_data.core_keywords,
                embedding=vector_data, 
                source_event_ids=[new_fact.event_id]
            )
            db.add(new_memory)
...
def get_pipeline_status(db: Session, page: int = 1, limit: int = 20, start_date: str = None, end_date: str = None, status_filter: str = None) -> dict:
    base_query = db.query(EventRaw).filter(EventRaw.sync_status_id != 9)
    if start_date:
    ...
    return {
        ...
        "data_list": data_list,
        "meta": { "total_count": total_list_count, "current_page": page, "total_pages": total_pages, "limit": limit }
    }