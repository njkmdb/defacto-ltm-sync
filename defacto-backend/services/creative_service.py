...
async def generate_creative_content(request: GenerateCreativeRequest, db: Session) -> dict:
    ...
    for src in request.sources:
        if src.source_type == "LOG":
            log = db.query(EventLog).filter(EventLog.log_id == src.source_id).first()
            if log: target_texts.append(f"[단기 일지 #{src.source_id}]\n{log.llm_summary}")
        elif src.source_type == "BRIEFING":
            briefing = db.query(EventBriefing).filter(EventBriefing.briefing_id == src.source_id).first()
            if briefing:
            ...
            creation = db.query(EventCreation).filter(EventCreation.creation_id == src.source_id).first()
            if creation: target_texts.append(f"[2차 창작물 #{src.source_id}]\n■ 원본 창작물 제목: {creation.creative_title}\n\n■ 창작물 내용:\n{creation.creative_content}")

    if not target_texts:
        raise Exception("유효한 원본 데이터 소스를 찾을 수 없습니다.")
    ...
    result = await lrse_client.extract_fact(
        ...
        target_schema_cls=CreativeContentSchema,
        system_instruction=final_instruction,
        temperature=request.temperature,
        max_tokens=request.max_length
    )
    
    if not result.fact_preservation_check:
        raise Exception("AI 자체 검증 결과, 원본 팩트가 훼손되거나 왜곡될 우려가 있어 창작이 거부되었습니다. 톤앤매너 프롬프트를 완화해 주세요.")
...
def get_event_creations(db: Session, page: int = 1, limit: int = 20, base_entity_id: int = None, start_date: str = None, end_date: str = None, search_conditions: str = None) -> dict:
    ...
    if base_entity_id:
        query = query.filter(EventCreation.base_entity_id == base_entity_id)
    ...
    for r in rows:
        ...
        data.append({
            ...
            "creative_content": r.creative_content,
            "ne_ts": r.ne_ts
        })
...
def delete_event_creation(creation_id: int, db: Session) -> dict:
    try:
        creation = db.query(EventCreation).filter(EventCreation.creation_id == creation_id).first()
        if not creation:
            raise Exception("해당 창작물을 찾을 수 없습니다.")

        # 💡 [App-level Cascade] 3차 창작물의 참조 배열에서 부모/조부모 creation_id 동기화 제거
        ...
        for c in linked_creations:
            c.source_creation_ids = [cid for cid in c.source_creation_ids if cid != creation_id]
...
async def generate_creative_meta_prompt(request: GenerateMetaPromptRequest) -> dict:
    meta_instruction = (
        "당신은 최고 수준의 AI 프롬프트 엔지니어입니다. "
        "사용자가 요구하는 [톤앤매너/스타일]을 바탕으로, 다른 LLM이 2차 창작을 수행할 때 사용할 '시스템 프롬프트'를 작성하십시오.\n"
    ...
    result = await lrse_client.extract_fact(
        raw_content=f"[사용자 요구 톤앤매너]: {request.user_intent}",
        target_schema_cls=MetaPromptSchema,
...
def get_event_creation(creation_id: int, db: Session) -> dict:
    r = db.query(EventCreation).filter(EventCreation.creation_id == creation_id).first()
    if not r:
        raise Exception("해당 창작물을 찾을 수 없습니다.")
