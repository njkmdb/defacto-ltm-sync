...
async def search_memory_explorer(request: MemorySearchRequest, db: Session) -> MemorySearchResponse:
    try:
        ...
        results, meta = rag_service.explore_memories(
            query_embedding=query_vector, page=request.page, limit=request.limit, distance_threshold=request.distance_threshold,
            base_entity_id=request.base_entity_id, search_conditions=request.search_conditions, include_dwh=request.include_dwh
        )
...
async def generate_event_briefing(request: GenerateBriefingRequest, db: Session, target_lang: str = "Korean") -> dict:
    try:
        ...
        index_texts = []
        for mem in memories:
            if mem.source_event_ids: all_source_ids.update(mem.source_event_ids)
        ...
        if plan_result.tool_calls:
            for tool_call in plan_result.tool_calls:
                if tool_call.tool_name == "FETCH_FACT_DETAILS": required_event_ids.update(tool_call.target_ids)
                elif tool_call.tool_name == "FETCH_ENTITY_MASTER" and tool_call.target_ids: additional_context_payload += f"\n\n{execute_fetch_entity_master(db, tool_call.target_ids)}"
                elif tool_call.tool_name == "FETCH_OBJECT_MASTER" and tool_call.target_ids: additional_context_payload += f"\n\n{execute_fetch_object_master(db, tool_call.target_ids)}"
        ...
        return {"status": "success", "message": "리포트 생성이 완료되었습니다.", "data": report_data.model_dump()}
    except Exception as e:
...
def save_event_briefing(request: SaveBriefingRequest, db: Session) -> dict:
    try:
        new_report = EventBriefing(
            base_entity_id=request.base_entity_id, query_text=request.query_text, executive_summary=request.executive_summary,
            key_findings=request.key_findings, risk_and_warnings=request.risk_and_warnings,
            recommended_actions=request.recommended_actions, source_memory_ids=request.source_memory_ids
        )
        db.add(new_report)
        db.commit()
...
def get_event_briefings(db: Session, page: int = 1, limit: int = 20, base_entity_id: int = None, start_date: str = None, end_date: str = None, search_conditions: str = None) -> dict:
    ...
    if base_entity_id: query = query.filter(EventBriefing.base_entity_id == base_entity_id)
    if start_date:
    ...
    for r in rows:
        data.append({
            "briefing_id": r.briefing_id, "base_entity_id": r.base_entity_id, "query_text": r.query_text,
            "executive_summary": r.executive_summary, "key_findings": r.key_findings or [], "risk_and_warnings": r.risk_and_warnings or [],
            "recommended_actions": r.recommended_actions or [], "source_memory_ids": r.source_memory_ids or [], "up_ts": r.up_ts, "ne_ts": r.ne_ts
        })
    return {"status": "success", "data": data, "meta": { "total_count": total, "current_page": page, "total_pages": total_pages, "limit": limit }}

def get_briefing_audit_trail(briefing_id: int, base_entity_id: int, db: Session) -> dict:
    report = db.query(EventBriefing).filter(EventBriefing.briefing_id == briefing_id, EventBriefing.base_entity_id == base_entity_id).first()
    if not report: raise Exception("해당 리포트를 찾을 수 없거나 접근 권한이 없습니다.")
    if not report.source_memory_ids: return {"status": "success", "data": []}
    ...
    data = [{"memory_id": m.memory_id, "content_text": m.content_text, "event_date": m.event_date, "source_event_ids": m.source_event_ids or []} for m in memories]
    return {"status": "success", "data": data}

def get_event_briefing(briefing_id: int, base_entity_id: int, db: Session) -> dict:
    r = db.query(EventBriefing).filter(EventBriefing.briefing_id == briefing_id, EventBriefing.base_entity_id == base_entity_id).first()
    if not r: raise Exception("해당 리포트를 찾을 수 없거나 접근 권한이 없습니다.")
    return {
        "status": "success",
        "data": {
            "briefing_id": r.briefing_id, "base_entity_id": r.base_entity_id, "query_text": r.query_text,
...
def update_event_briefing(briefing_id: int, request: UpdateBriefingRequest, db: Session) -> dict:
    try:
        r = db.query(EventBriefing).filter(EventBriefing.briefing_id == briefing_id, EventBriefing.base_entity_id == request.base_entity_id).first()
        if not r: raise Exception("해당 리포트를 찾을 수 없거나 권한이 없습니다.")
...
def delete_event_briefing(briefing_id: int, base_entity_id: int, db: Session) -> dict:
    try:
        r = db.query(EventBriefing).filter(EventBriefing.briefing_id == briefing_id, EventBriefing.base_entity_id == base_entity_id).first()
        if not r: raise Exception("해당 리포트를 찾을 수 없거나 권한이 없습니다.")
        linked_creations = db.query(EventCreation).filter(EventCreation.source_briefing_ids.contains([briefing_id])).all()
        for c in linked_creations: c.source_briefing_ids = [bid for bid in c.source_briefing_ids if bid != briefing_id]
        db.delete(r)
        db.commit()
...
def delete_bulk_event_briefings(briefing_ids: list[int], base_entity_id: int, db: Session) -> dict:
    try:
        if briefing_ids:
            briefings_to_delete = db.query(EventBriefing).filter(EventBriefing.briefing_id.in_(briefing_ids), EventBriefing.base_entity_id == base_entity_id).all()
            valid_ids = [b.briefing_id for b in briefings_to_delete]
            if not valid_ids: return {"status": "success", "message": "삭제할 대상이 없습니다."}
            ids_csv = ",".join(f"'{bid}'" for bid in valid_ids)
            linked_creations = db.query(EventCreation).filter(text(f"EXISTS (SELECT 1 FROM jsonb_array_elements_text(source_briefing_ids) AS elem WHERE elem IN ({ids_csv}))")).all()
            for c in linked_creations: c.source_briefing_ids = [bid for bid in c.source_briefing_ids if bid not in valid_ids]
            db.query(EventBriefing).filter(EventBriefing.briefing_id.in_(valid_ids)).delete(synchronize_session=False)
            db.commit()
        return {"status": "success", "message": f"총 {len(valid_ids)}건의 리포트가 일괄 삭제되었습니다."}
    except Exception as e:
        db.rollback()
