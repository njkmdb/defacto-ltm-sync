...
from schemas.api_schemas import ContextSynthesisSchema, AgentPlanningSchema
...
# 💡 [핵심 교정] 온도(temperature) 및 길이(max_length) 반환을 위해 시그니처 수정
def get_dynamic_prompt(db: Session, step: str, entity_id: int, default_schema_name: str) -> tuple[str, Any, float, int]:
    ...
    if selected:
        ...
        # 💡 설정된 동적 값(Temp, MaxLength)을 함께 반환
        return (
