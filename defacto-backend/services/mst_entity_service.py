...
def get_mst_entities(db: Session, page: int = 1, limit: int = 20, type_filter: str = None, search_conditions: str = None) -> dict:
    query = db.query(MstEntity).filter(MstEntity.entity_status_id != 9, MstEntity.entity_id != 0)
    
    if type_filter and type_filter != 'ALL': 
        query = query.filter(MstEntity.entity_type == type_filter)
    ...
    if search_conditions:
        try:
            ...
            logger.error(f"다중 검색 파싱 오류 (Entity): {e}")
            pass
    ...
    data = [{"entity_id": e.entity_id, "parent_entity_id": e.parent_entity_id, "entity_type": e.entity_type, "entity_name": e.entity_name, "attributes": e.attributes or {}, "entity_status_id": e.entity_status_id, "ne_ts": e.ne_ts, "up_ts": e.up_ts} for e in entities]
...
def create_mst_entity(request: CreateMstEntityRequest, db: Session) -> dict:
    try:
        if request.entity_id is not None:
            ...
            if existing: 
                raise Exception(f"이미 사용 중인 주체 ID ({request.entity_id}) 입니다.")
        ...
        kwargs = {
            ...
            "entity_status_id": request.entity_status_id or 1
        }
        if request.entity_id is not None: 
...
def update_mst_entity(entity_id: int, request: UpdateMstEntityRequest, db: Session) -> dict:
    try:
        ...
        if request.entity_status_id is not None: 
            entity.entity_status_id = request.entity_status_id
        ...
        for item in request.items:
            if item.entity_id is not None:
                stmt = insert(MstEntity).values(
                    ...
                    entity_status_id=item.entity_status_id or 1, 
