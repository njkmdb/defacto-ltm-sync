...
def delete_event_log(log_id: int, db: Session) -> dict:
    try:
        log_record = db.query(EventLog).filter(EventLog.log_id == log_id).first()
        if not log_record: return {"status": "error", "message": f"일지 ID {log_id} 미발견"}

        linked_creations = db.query(EventCreation).filter(EventCreation.source_log_ids.contains([log_id])).all()
        for c in linked_creations:
            c.source_log_ids = [lid for lid in c.source_log_ids if lid != log_id]
...
def delete_bulk_event_logs(log_ids: list[int], db: Session) -> dict:
    try:
        # 💡 [AST Depth Limit 최적화] or_ 전개 대신 EXISTS와 jsonb_array_elements를 활용한 단일 쿼리로 최적화
        if log_ids:
            ids_csv = ",".join(f"'{lid}'" for lid in log_ids)
            linked_creations = db.query(EventCreation).filter(
                text(f"EXISTS (SELECT 1 FROM jsonb_array_elements(source_log_ids) AS elem WHERE elem::text IN ({ids_csv}))")
            ).all()
            for c in linked_creations:
                c.source_log_ids = [lid for lid in c.source_log_ids if lid not in log_ids]

        db.query(EventLog).filter(EventLog.log_id.in_(log_ids)).delete(synchronize_session=False)
        db.commit()
        return {"status": "success", "message": f"총 {len(log_ids)}건의 일지가 삭제되었습니다."}
    except Exception as e:
        db.rollback()
        raise Exception(str(e))
...
def save_edited_summary(request: SaveSummaryRequest, db: Session) -> dict:
    try:
        ...
        if request.log_id:
            existing = db.query(EventLog).filter(EventLog.log_id == request.log_id).first()
            if not existing: raise Exception("해당 일지를 찾을 수 없습니다.")
            existing.base_entity_id = request.base_entity_id
            ...
            if existing:
                ...
                max_id = db.query(func.max(EventLog.log_id)).scalar() or 0
                new_log = EventLog(
                    log_id=max_id + 1,
...
def bulk_upsert_event_logs(request: BulkUpsertLogRequest, db: Session) -> dict:
    try:
        ...
        current_max_id = db.query(func.max(EventLog.log_id)).scalar() or 0
        ...
        for item in request.items:
            ...
            if item.log_id is not None:
                ...
                update_dict = {
                    "base_entity_id": stmt.excluded.base_entity_id,
...
def get_event_log(log_id: int, db: Session) -> dict:
    log = db.query(EventLog).filter(EventLog.log_id == log_id).first()
    if not log:
        raise Exception("해당 일지를 찾을 수 없습니다.")
    ...
    return {
        ...
        "data": {
            "log_id": log.log_id, 
            "base_entity_id": log.base_entity_id, 
