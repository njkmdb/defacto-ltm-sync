...
def get_mst_objects(db: Session, page: int = 1, limit: int = 20, type_filter: str = None, search_conditions: str = None) -> dict:
    ...
    if type_filter and type_filter != 'ALL': 
        query = query.filter(MstObject.object_type == type_filter)
    ...
    if search_conditions:
        try:
            ...
            logger.error(f"다중 검색 파싱 오류 (Object): {e}")
            pass
...
def create_mst_object(request: CreateMstObjectRequest, db: Session) -> dict:
    try:
        if request.object_id is not None:
            ...
            if existing: 
                raise Exception(f"이미 사용 중인 객체 ID ({request.object_id}) 입니다.")
        ...
        kwargs = {
            ...
            "object_status_id": request.object_status_id or 1
        }
        if request.object_id is not None: 
            kwargs["object_id"] = request.object_id
...
def update_mst_object(object_id: int, request: UpdateMstObjectRequest, db: Session) -> dict:
    try:
        ...
        if request.object_status_id is not None: 
            obj.object_status_id = request.object_status_id
        ...
        for item in request.items:
            if item.object_id is not None:
                stmt = insert(MstObject).values(
                    ...
                    object_status_id=item.object_status_id or 1, 
                    up_ts=datetime.utcnow()
                )
                update_dict = {
                    "object_type": stmt.excluded.object_type, 
                    "object_name": stmt.excluded.object_name,
                    "parent_object_id": stmt.excluded.parent_object_id, 
                    "attributes": stmt.excluded.attributes,
                    "object_status_id": stmt.excluded.object_status_id, 
                    "up_ts": datetime.utcnow()
                }
